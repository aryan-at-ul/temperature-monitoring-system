...
from api.auth.token_auth import get_admin_user, check_read_permission
...
async def system_info(admin: dict = Depends(get_admin_user)):
# async def system_info(customer: dict = Depends(check_read_permission)):
    ...
    try:
        ...
        system_info = {
            ...
            "environment": {
                "hostname": platform.node(),
                "timezone": str(now.tzinfo),  # Convert to string
                "local_time": now.isoformat(),  # Add readable timestamp
                "utc_time": datetime.utcnow().isoformat() + "Z"  # Add UTC time
            }
        }
